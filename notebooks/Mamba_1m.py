...
def plot_losses(train_losses, val_losses, mode, args):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'../plots_1m/{args.mamba_variant}_loss.png')
    plt.close()
...
def plot_metrics(train_accs, val_accs, mode, args):
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(f'../plots_1m/{args.mamba_variant}_acc.png')
    plt.close()
...
def main(args):
    if args.mode == "gen":
        ...
        with open(f'../results_1m/{args.mamba_variant}_metrics.txt', 'w') as f:
